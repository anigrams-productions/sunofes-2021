init python:
    ...
    class Encounter:
        def __init__(self, theme, players, current_player):
            ...
            # make sure to add quest, objective, and boss scenarios after determining random scenarios
            self.scenario_quest = Scenario(self.theme, ScenarioType.Quest)
            self.scenario_boss = Scenario(self.theme, ScenarioType.Boss)
            self.scenario_objective = Scenario(self.theme, ScenarioType.Objective)
