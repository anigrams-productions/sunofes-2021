    ...
    class Scenario:
        def __init__(self, theme, scenario_type):
            ...
            if self.scenario_type == ScenarioType.Enemy:
                ...
                self.menu_label = warning_label
            elif self.scenario_type == ScenarioType.Objective:
                self.script = self.get_objective()
                self.menu_label = warning_label
        ...
        def get_enemy(self):
            ...
            return "t3_scenario_" + random_enemy.theme + "_enemy_" + random_enemy.enemy_type + "_" + random_enemy.enemy_id
        ...
        def get_objective(self):
            filtered_npcs = filter(lambda npc: npc.theme == self.theme, ObjectiveNpcs)
            self.current_entity = filtered_npcs[0]
            # return name of objective NPC script to run
            return "t3_scenario_" + self.theme + "_objective"
